...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.home_frame_button_1.grid(
            row=0, column=0, padx=20, pady=15, sticky="ew")
        ...
        self.home_frame_button_2.grid(
            row=3, column=0, padx=20, pady=15, sticky="ew")
        ...
        self.home_frame_button_3.grid(
            row=1, column=0, padx=20, pady=15, sticky="ew")
        ...
        # allow the history frame to expand columns and rows with widgets when
        # the frame is resized
        self.history_frame.grid_rowconfigure(1, weight=1)
        self.history_frame.grid_columnconfigure(0, weight=1)

        # create our clear history button and make it stay centered at the top
        # of the screen
        ...
        self.history_frame_clear_button.grid(
        ...
        self.history_textbox = customtkinter.CTkTextbox(
            self.history_frame, width=435, height=280)
        self.history_textbox.grid(
        ...
        self.text_decoder_label = customtkinter.CTkLabel(self.settings_frame, text="Text Decoder",
                                                         font=customtkinter.CTkFont(size=15, weight="bold"))
        self.text_decoder_label.grid(
            row=0, column=0, padx=0, pady=0, sticky="n")

        # create settings button for setting up the text decoder
        self.settings_button1 = customtkinter.CTkSegmentedButton(
            self.settings_frame, command=self.api_button_handler)
        self.settings_button1.configure(
            values=["PyTesseract", "Google Vision"])
        self.settings_button1.set("PyTesseract")
        self.settings_button1.grid(
        ...
        self.keybind_button_clipboard.grid(
            row=18, column=0, padx=20, pady=0, sticky="ew")

        self.keybind_textbox_clipboard = customtkinter.CTkTextbox(
            self.settings_frame, width=435, height=25)
        self.keybind_textbox_clipboard.grid(
            row=19, column=0, padx=20, pady=0, sticky='ew')
        ...
        self.text_decoder_label = customtkinter.CTkLabel(self.settings_frame, text="API Credentials Path",
                                                         font=customtkinter.CTkFont(size=15, weight="bold"))
        self.text_decoder_label.grid(
            row=10, column=0, padx=0, pady=0, sticky="n")

        self.filepath_textbox = customtkinter.CTkTextbox(
            self.settings_frame, width=435, height=25)
        self.filepath_textbox.grid(
            row=13, column=0, padx=20, pady=0, sticky='ew')
        ...
        self.filepath_button.grid(
            row=12, column=0, padx=20, pady=0, sticky="ew")
        ...
        self.keybind_textbox = customtkinter.CTkTextbox(
            self.settings_frame, width=435, height=25)
        self.keybind_textbox.grid(
            row=16, column=0, padx=20, pady=0, sticky='ew')
        ...
        self.keybind_button.grid(
            row=15, column=0, padx=20, pady=0, sticky="ew")
    ...
    def load_settings(self):
        ...
        if os.path.isfile(self.SETTINGS_FILE):
            with open(self.SETTINGS_FILE, 'r') as f:
                ...
                # hotkey setup
                self.clipboard_hotkey = keyboard.add_hotkey(
                    settings['clipboardKeybind'], self.queue_copy_clipboard)
                self.screenshot_hotkey = keyboard.add_hotkey(
                    settings['screenshotKeybind'], self.queue_take_screenshot)
                ...
                self.keybind_textbox_handler_clipboard(
                    settings['clipboardKeybind'])
                self.keybind_textbox.configure(state=tk.DISABLED)
                self.keybind_textbox_clipboard.configure(state=tk.DISABLED)
                ...
                # Insert the filepath into the textbox
                self.filepath_textbox.insert('1.0', self.credentials)
                self.filepath_textbox.configure(state=tk.DISABLED)
            ...
            default_settings = {
                'clipboardKeybind': 'control+shift',
                'screenshotKeybind': 'alt+shift',
                'decoder': 'PyTesseract',
                'apiCredentials': '',
            ...
            self.clipboard_hotkey = keyboard.add_hotkey(
                self.settings['clipboardKeybind'], self.queue_copy_clipboard)
            self.screenshot_hotkey = keyboard.add_hotkey(
                self.settings['screenshotKeybind'], self.queue_take_screenshot)
    ...
    def create_sys_tray_icon(self):
        ...
        menu = (
            item(
                'Show', self.show_from_tray), item(
                'Quit', self.stop_program))
    ...
    def capture_text_button(self):
        ...
        for idx, screen in enumerate(get_monitors()):
            window = self.screenshot_handler.create_image_window(
                screen, images[idx], idx)
    ...
    def select_frame_by_name(self, name):
        ...
        self.home_button.configure(
            fg_color=(
                "gray75",
                "gray25") if name == "home" else "transparent")
        self.history_button.configure(
            fg_color=(
                "gray75",
                "gray25") if name == "history_frame" else "transparent")
        self.settings_button.configure(
            fg_color=(
                "gray75",
                "gray25") if name == "settings_frame" else "transparent")
    ...
    def record_keybind_button_hanlder(self):
        ...
        self.screenshot_hotkey = keyboard.add_hotkey(
            keybind, self.queue_take_screenshot)
    ...
    def record_keybind_button_hanlder_clipboard(self):
        ...
        self.clipboard_hotkey = keyboard.add_hotkey(
            keybind, self.queue_copy_clipboard)
